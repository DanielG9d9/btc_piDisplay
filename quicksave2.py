# Latest with blockchain keeping code WIP
...
testing = False # Turn to true when testing so you don't ping for price a lot
...
def update_blockchain_info():
    global last_blockchain_update, blockchain_chain, blockchain_blocks, blockchain_verification_progress, node_connections, cpu_temp, ax, canvas
    ...
    if current_time - last_blockchain_update >= 60: # 600 seconds = 10 minutes
        try:
            ...
            node_subversion = f"{network_info['subversion']}"
            node_connections_in, node_connections_out = f"{network_info['connections_in']}", f"{network_info['connections_out']}"
