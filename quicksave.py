...
def update_price_chart():
    ...
    if current_time - last_price_update >= 60:  # Update every hour
        print("yes!")
        try:
            ...
            if prices:
                ...
                if daily_change >= 0:
                    ax.set_title(f"Bitcoin Price: ${current_price:,.0f}) - 24h Change: +{daily_change}%", color='white', loc='left', fontsize=16)
                else:
                    ax.set_title(f"Bitcoin Price: ${current_price:,.0f}) - 24h Change: -{daily_change}%", color='white', loc='left', fontsize=16)
...
def update_blockchain_info():
    global last_blockchain_update, blockchain_chain, blockchain_blocks, blockchain_verification_progress, node_connections, cpu_temp

    current_time = time.time()
    # print(f"Update Blockchain info?: {current_time - last_blockchain_update} >= 600")
    if current_time - last_blockchain_update >= 60: # 600 seconds = 10 minutes
        try: # Let's update info
            def get_node_info(rpc_connection):
                ...
                # print(network_info)
                low_fee, medium_fee, high_fee = get_fee_estimates(rpc_connection)
                return blockchain_info, network_info, low_fee, medium_fee, high_fee
            new_chain_info, new_network_info, low_fee, medium_fee, high_fee = get_node_info(rpc_connection)

            def update_node_table(blockchain_data, network_data):
                ...
                #TODO: node_connections = f"{network_data['connections']}"
                #TODO: node_subversion = f"{network_data['subversion']}"
                #TODO: node_connections_in, node_connections_out = f"{network_data['connections_in']}", f"{network_data['connections_out']}"
            ...
            previous_chain = new_chain_info
            previous_network = new_network_info
...
def create_display(rpc_connection):
    ...
    def update_display():
        ...
        root.after(60000 * 2, update_display)  # Schedule next price update in 1 minute
            
    # Initial calls when launching app
    update_price_chart()
    root.after(100, update_blockchain_info)  # Start blockchain update after 100ms   
    root.after(200, update_display)  # Schedule regular updates 
    return root
